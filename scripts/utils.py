...
def weighted_loss(class_weights):
    ...
    def weighted_binary_crossentropy(y_true, y_pred):
        ...
        return K.sum(K.binary_crossentropy(y_true, y_pred)* weight_values)/len(weight_values)
...
def verify_model(model, x, y, reverse_data_dictionary, usage_scores, standard_conn, lowest_tool_ids, topk_list=[1, 2, 3]):
    ...
    for i in range(size):
        ...
        for index, abs_topk in enumerate(topk_list):
            usg_wt_score, absolute_precision, pub_prec, lowest_p_prec, lowest_n_prec = compute_precision(model, test_sample, y, reverse_data_dictionary, usage_scores, actual_classes_pos, abs_topk, standard_conn, last_tool_id, lowest_tool_ids,actual_next_tool_names)
